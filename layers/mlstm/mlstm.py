...
from layers.mlstm.components.conv import CausalConv1d, CausalConv1dConfig
from layers.mlstm.components.init import small_init_init_, wang_init_
from layers.mlstm.components.linear_headwise import (
    LinearHeadwiseExpand,
    LinearHeadwiseExpandConfig,
)
from layers.mlstm.utils import UpProjConfigMixin
from layers.mlstm.cell import mLSTMCell, mLSTMCellConfig
...
if __name__ == "__main__":
    # 定义随机输入张量，形状为 (batch_size=2, seq_length=32, embedding_dim=64)
    x = torch.randn(2, 32, 64)

    # 初始化 mLSTMLayer 配置
    config = mLSTMLayerConfig(
        embedding_dim=64,         # 输入嵌入的维度
        conv1d_kernel_size=4,     # 一维卷积核的大小
        qkv_proj_blocksize=4,     # QKV投影的块大小
        num_heads=4,              # mLSTM的头数
        proj_factor=2.0,          # 上投影的比例因子
        bias=True,                # 是否使用偏置
